...
class DomainAnalyzer:
    ...
    def _extract_permissions(
        self, text: str, operations: list[str], objects: list[str]
    ) -> list[str]:
        permissions: set[str] = set()
        # Look for explicit permission mentions
        for match in re.finditer(
            r"([A-Z_]+_PERMISSION|[A-Z_]+_DELETE|[A-Z_]+_UPLOAD|[A-Z_]+_VIEW)",
            text,
        ):
    ...
    def _extract_business_objects(self, text: str, spec_objects: list[str]) -> list[str]:
        ...
        for match in re.finditer(
            r"\b([a-z]+\s+(?:log|record|document|object|item|entry|history))\b",
            text,
            re.IGNORECASE,
        ):
