...
class RequirementParser:
    ...
    KNOWN_ROLES = {
        "user", "admin", "manager", "sales", "procurement", "inventory",
        "business_user", "authorized_user", "system_admin", "customer",
        "vendor", "supplier", "buyer", "seller", "approver", "reviewer",
        "operator", "analyst", "developer", "tester", "product_manager",
        "project_manager", "team_lead", "engineer", "consultant",
        "account_manager", "support", "helpdesk", "auditor",
    }

    # Common non-business words that should NOT be objects
    STOP_WORDS = {
        "system", "feature", "function", "functionality", "service",
        "application", "app", "platform", "tool", "module", "component",
        "capability", "ability", "option", "setting", "configuration",
        "process", "workflow", "operation", "action", "activity", "task",
        "data", "information", "record", "entry", "item", "object",
        "should", "must", "can", "will", "would", "could", "may",
        "only", "also", "even", "just", "all", "each", "every",
        "proper", "valid", "invalid", "correct", "incorrect",
        "unified", "comprehensive", "integrated", "advanced", "basic",
        "new", "existing", "current", "previous", "next", "first", "last",
        "one", "two", "multiple", "various", "different", "same", "other",
        "supporting", "relevant", "necessary", "required", "optional",
    }
    ...
    def _extract_actors(self, text: str) -> List[str]:
        ...
        for match in re.finditer(r"As\s+a[n]?\s+([a-zA-Z_]+(?:\s+[a-zA-Z_]+){0,2})", text, re.IGNORECASE):
        ...
        for match in re.finditer(r"^([A-Z][a-zA-Z]+(?:\s+[a-zA-Z][a-zA-Z]+){0,2})\s+should\s+(?:be\s+)?able\s+to", text, re.MULTILINE | re.IGNORECASE):
            role = match.group(1).strip().lower().replace(" ", "_")
        ...
        for match in re.finditer(r"Only\s+([a-z]+(?:\s+[a-z]+){0,2})\s+(?:can|may|should)", text, re.IGNORECASE):
            role = match.group(1).strip().lower().replace(" ", "_")
            if role and role not in self.STOP_WORDS and len(role) > 2:
                roles.add(role)
        ...
        if not roles:
            for known in self.KNOWN_ROLES:
                if re.search(rf"\b{known.replace('_', ' ')}\b", text, re.IGNORECASE) or \
                   re.search(rf"\b{known}\b", text, re.IGNORECASE):
                    roles.add(known)
    ...
    def _extract_features(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            match2 = re.search(r"should\s+(provide|support|expose|record|enforce|maintain|generate|send|validate|check)\s+(.+)", line, re.IGNORECASE)
            if match2:
    ...
    def _split_compound_actions(self, text: str) -> List[str]:
        """Split 'upload, preview, download, print, and delete attachments' into individual actions."""
        text = text.rstrip(".")
                ...
                for n in (2, 1):
                    if len(words) >= n:
                        ...
                        if candidate and len(candidate) > 3 and candidate not in self.STOP_WORDS:
                            # Validate: must be a noun-like word
                            if not candidate.endswith(("ing", "ed", "ly")):
                                objects.add(candidate.replace(" ", "_"))
                                break

        # Known ERP objects from text
        erp_objects = [
            "attachment", "document", "file", "customer", "order", "sales_order",
            "purchase_order", "invoice", "payment", "product", "inventory", "warehouse",
            "user", "role", "permission", "workflow", "approval", "notification",
            "audit_log", "log", "quote", "contract", "supplier", "vendor", "account",
            "contact", "lead", "opportunity", "ticket", "issue", "project", "task",
            "employee", "department", "company", "organization", "team",
            "report", "dashboard", "metric", "kpi", "event", "calendar", "meeting",
            "email", "message", "comment", "note", "tag", "category", "label",
        ]
